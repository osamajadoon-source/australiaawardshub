...
SEARCH_QUERIES = [
    ...
    '"PhD position" available students',
    '"graduate research scholarship" apply',
    '"research training program" scholarship apply',
]
...
def google_cse_search(query, api_key, cse_id, num=10):
    site_filter = " OR ".join(f"site:{d}" for d in UNI_DOMAINS)
    ...
    params = {
        ...
        "q": full_query,
        "num": min(num, 10),
        "dateRestrict": "d14",  # only listings indexed in the last 14 days
    }
    url = "https://www.googleapis.com/customsearch/v1?" + urllib.parse.urlencode(params)
    req = urllib.request.Request(url, headers={"User-Agent": "australiaawardshub-phd-finder/1.0"})
...
def pick_new_candidate(posted_urls, api_key, cse_id):
    ...
    for q in queries:
        try:
            results = google_cse_search(q, api_key, cse_id)
        ...
        for item in results.get("items", []):
            link = item.get("link", "")
            if link in posted_urls or is_excluded(link):
                continue
            ...
            return {
                ...
                "url": link,
                "snippet": item.get("snippet", "").strip()[:280],
...
def main():
    ...
    if not api_key or not cse_id:
        print("Missing GOOGLE_CSE_API_KEY or GOOGLE_CSE_ID — add both as repo secrets.", file=sys.stderr)
        sys.exit(1)

    posted = load_posted()
    posted_urls = {e["url"] for e in posted}

    candidate = pick_new_candidate(posted_urls, api_key, cse_id)
    if not candidate:
        print("No new candidate found today — nothing to commit.")
        return

    posted.append(candidate)
    save_posted(posted)
    update_html(posted)
    print(f"Added new listing: {candidate['title']} ({candidate['university']}) -> {candidate['url']}")
